...
-- COLOR THEMES --
-- Options: "retro_amber" (custom), "catppuccin-frappe", "catppuccin-mocha"
local theme = "retro_amber"

if theme == "retro_amber" then
	-- Load custom retro amber theme
	require("colors.retro_amber").setup()

	local c = require("colors.retro_amber").colors

	-- Debugger signs
	vim.api.nvim_set_hl(0, "amber", { fg = c.amber })
	vim.api.nvim_set_hl(0, "amber_bright", { fg = c.amber_bright })
	vim.api.nvim_set_hl(0, "amber_dim", { fg = c.amber_dim })
	vim.api.nvim_set_hl(0, "error_hl", { fg = c.error })

	vim.fn.sign_define(
		"DapBreakpointCondition",
		{ text = "•", texthl = "amber", linehl = "DapBreakpoint", numhl = "DapBreakpoint" }
	)
	vim.fn.sign_define(
		"DapBreakpointRejected",
		{ text = "•", texthl = "error_hl", linehl = "DapBreakpoint", numhl = "DapBreakpoint" }
	)
	vim.fn.sign_define(
		"DapLogPoint",
		{ text = "•", texthl = "amber_bright", linehl = "DapBreakpoint", numhl = "DapBreakpoint" }
	)
else
	-- Catppuccin theme
